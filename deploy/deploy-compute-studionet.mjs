...
async function main() {
  ...
  for (const [id, data] of providers) {
    await callWrite(oracleAddr, 'register_provider', [id, JSON.stringify(data)], `Register ${id}`);
  }
  ...
  // Step 3: Deploy ComputeRouterFull with oracle address
  const routerAddr = await deployContract(
    'ComputeRouterFull',
    resolve(root, 'contracts/compute_router_full.py'),
    [account.address]
  );
